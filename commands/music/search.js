...
module.exports = {
    ...
    async execute({ client, inter }) {
        ...
        .setColor('#ff0000')
        .setAuthor({ name: `Résultats pour ${chanson}`, iconURL: client.user.displayAvatarURL({ size: 1024, dynamic: true })})
        ...
        collector.on('collect', async (query) => {
            ...
            if (!value || value <= 0 || value > maxTracks.length) return inter.followUp({ content: `Réponse invalide, essayez une valeur entre **1** et **${maxTracks.length}** ou **cancel**... réessayez ? ❌`, ephemeral: true });
            ...
            try {
                ...
                return inter.followUp({ content: `Je n'arrive pas à rejoindre le canal vocal ${inter.member}... réessayer ? ❌`, ephemeral: true });
            }
        ...
        collector.on('end', (msg, reason) => {
            if (reason === 'time') return inter.followUp({ content:`La recherche a expiré ${inter.member}... réessayez ? ❌`, ephemeral: true })
        });
    },
};