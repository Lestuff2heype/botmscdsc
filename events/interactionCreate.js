...
module.exports = (client, inter) => {
    ...
    if (!command) return inter.reply({ embeds: [ new EmbedBuilder().setColor('#ff0000').setDescription('❌ | Erreur! Veuillez contacter les développeurs !')], ephemeral: true, }), client.slash.delete(inter.commandName)
    ...
    if (DJ.enabled && DJ.commands.includes(command) && !inter.member._roles.includes(inter.guild.roles.cache.find(x => x.name === DJ.roleName).id)) return inter.reply({ embeds: [ new EmbedBuilder().setColor('#ff0000').setDescription(`❌ | Cette commande est réservée aux membres ayant le rôle: \`${DJ.roleName}\` `)], ephemeral: true, })
